...
const CartItemCard = ({ item, index, isOutOfStock, user, cartUpdated, setCartUpdated, handleProduct, handleQuantityChange, deleteCartItem , fallbackImg }) => {
    ...
    return (
        <div className="border-b-2 p-4 border-black flex gap-2 justify-between">
                ...
                <div className="text-lg font-semibold flex gap-2 flex-col">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 w-full">
                        {!isOutOfStock && (
                            <div className="flex gap-4 items-center border-2 border-yellow-400 rounded-2xl py-1 px-3 w-full sm:w-auto text-sm">
                                {item?.quantity > 1 ? (
                                    <p
                                        onClick={() =>
                                            user ? decreaseQuantity(user._id, item?._id, cartUpdated, setCartUpdated)
                                    ...
                                    <p
                                        onClick={() =>
                                            user ? deleteCartItem(user._id, item?._id, cartUpdated, setCartUpdated)
                                ...
                                <p
                                    onClick={() =>
                                        user ? increaseQuantity(user._id, item?._id, cartUpdated, setCartUpdated)
                        ...
                        <div
                            ...
                            onClick={() => user ? deleteCartItem(user._id, item?._id, cartUpdated, setCartUpdated)
                                : handleQuantityChange(item._id, "remove")}
                        >
                            <Trash2 className="w-4 h-4 sm:w-5 sm:h-5" />
                            <p>Delete</p>
